...
class Torch_model_interf:
    def __init__(self, model_file, mode, image_size=(112, 112)):
        import torch
        ...
        try:
            ...
            backbone = load_model(backbone, model_file)
...
def get_embeddings(model_interf, img_names, landmarks, batch_size=64, flip=True):
    ...
    # np.savez('./IJB_result/embs.npz', np.array(embs))
    # np.savez('./IJB_result/embs_f.npz', np.array(embs_f))
...
def verification_11(template_norm_feats=None, unique_templates=None, p1=None, p2=None, batch_size=10000):
    ...
    sublists = [
        total_pairs[i:i + batch_size] for i in range(0, len(p1), batch_size)
    ]
...
class IJB_test:
    def __init__(self, model_file, data_path, subset, batch_size=64, force_reload=False, restore_embs=None, mode=None):
        ...
        if model_file != None:
            if model_file.endswith(".h5"):
                ...
                interf_func = Torch_model_interf(model_file, mode=mode)
...
def parse_arguments(argv):
    ...
    default_save_result_name = "IJB_result/{model_name}_{subset}_{type}_modified.npz"
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("-c", "--config_file", type=str, default='/home/k64t/face_recognition/configs/ijbb.yaml', help="Path to config file.")
    parser.add_argument("-m", "--model_file", type=str, default='/home/k64t/face_recognition/save/magface-26/ckpt/magface-26_21.pth', help="Saved model, keras h5 / pytorch jit pth / onnx / mxnet")
    parser.add_argument("-d", "--data_path", type=str, default="/home/k64t/face_recognition/data/ijb", help="Dataset path containing IJBB and IJBC sub folder")
...
if __name__ == "__main__":
    ...
    if args.plot_only != None and len(args.plot_only) != 0:
        ...
        for model_path in cfg['weight_path']:
            ...
            tt = IJB_test(cfg['weight_path'][model_path], args.data_path, args.subset, args.batch_size, args.force_reload, args.save_result, cfg['module'][model_path])
            if args.save_embeddings:  # Save embeddings first, in case of any error happens later...
                np.savez(f'IJB_result/{os.path.splitext(os.path.basename(model_path))[0]}_embedding.npz', embs=tt.embs, embs_f=tt.embs_f)
            ...
            if args.model_file != None or args.save_embeddings:  # embeddings not restored from file or should save_embeddings again
                ...
                np.savez('IJB_result/{model_name}_{subset}_{type}_modified.npz'.format(model_name=os.path.splitext(os.path.basename(model_path))[0], subset=args.subset, type=type), **save_items)
